...
const Posts = ({ user, posts, setPosts, getPosts }) => {
  ...
  const showPosts = posts.map((post) => {
    console.log(`post in map: ${post._id}`);
    return (
      <Post
        key={post._id}
        id={post._id}
    ...
    <>
      {user ? (
        <>
          <div className="post-form">
            <form onSubmit={handleMessageSubmit}>
              ...
              <div className="post-btn-section">
                ...
                <input id="real-btn" type="file" hidden="hidden"></input>
                <i id="custom-button" className="fas fa-image fa-2x"></i>
                &nbsp;&nbsp;&nbsp;&nbsp;
          ...
          <div className="left">
            <button className="refresh-btn" onClick={() => getPosts()}>
              <i className="fas fa-sync fa-1.5x"></i>&nbsp;Refresh
            </button>
          </div>
        </>
      ) : (
